...
def generate_today_ai_trending(model_type, model_name):
    config.llm_model_type = model_type
    ...
    # 定义一个函数，用于导出和生成指定时间范围内项目的进展报告
    raw_file_path = aibase_client.export_today_ai_trending()  # 导出原始数据文件路径
    report, report_file_path = report_generator.generate_aibase_report(raw_file_path)  # 生成并获取报告内容及文件路径
...
def update_model_list(model_type):
    if model_type == "openai":
        ...
        return gr.Dropdown(choices=["llama3.1:8b", "gemma2:2b", "qwen2:7b"], label="选择模型")
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    # 创建 AIBase Today's AI Trending 热点话题 Tab
    with gr.Tab("AIBase 今日 AI 热点"):
        gr.Markdown("## 今日 AI 热点")  # 添加小标题
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_today_ai_trending, inputs=[model_type, model_name,], outputs=[markdown_output, file_output])
